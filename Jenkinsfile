           ...
           stage('Start API Pods') {
            steps {
                script {
                        ...
                        sh "kubectl logs e2e-test-app-job-6rv44 -n filetracker"
                        sh "kubectl logs e2e-test-app-job-7msc9 -n filetracker"
...
def waitForReport() {
    timeout(time: 5, unit: 'MINUTES') {
        script {
            def counter = 0 
            while (!fileExists('/shared/cypress/reports/html/index.html')) {
                sh "kubectl get all -n filetracker"
                counter++ 
